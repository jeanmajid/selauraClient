...
namespace selaura {
    void environment::on_enable() {
        ...
        evm.subscribe(&environment::on_timeofday_event, this);
    }

    void environment::on_disable() {
        auto& evm = selaura::get()->get<selaura::event_manager>();
        evm.unsubscribe(&environment::on_timeofday_event, this);
    }

    void environment::on_timeofday_event(selaura::getTimeOfDay_event &event) {
